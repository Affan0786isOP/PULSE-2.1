...
export interface BmtStats { medianSpan: number | null; maxSpan: number | null; meanInterTapRt: number | null; successRate: number | null; spanDistribution: { span: number; count: number; percentage: number; }[]; progressionCurve: { level: number; attemptCount: number; accuracyRate: number | null; }[]; }
