...
export function NotFound({ onNavigate }: { onNavigate: (view: string) => void }) {
  return (
    <div className="min-h-[100dvh] bg-transparent text-[var(--text-primary)] font-sans selection:bg-cyan-500/30 flex flex-col relative overflow-hidden">
      <Navbar currentView="" onNavigate={onNavigate} />
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 z-10 text-center">
        ...
        <button type="button" 
          ...
          className="bg-[var(--surface-1)] hover:bg-[var(--surface-2)] border border-[var(--border-subtle)] hover:border-[var(--accent)] rounded-lg px-6 py-3 flex items-center gap-2.5 transition-colors group active:scale-95 cursor-pointer text-xs font-semibold text-[var(--text-primary)]"
        >
          <span>Return to Dashboard</span>
          <ArrowRight size={14} className="text-[var(--text-muted)] group-hover:text-[var(--accent)] group-hover:translate-x-0.5 transition-[color,transform]" />
        </button>
      </main>
    </div>
  );
