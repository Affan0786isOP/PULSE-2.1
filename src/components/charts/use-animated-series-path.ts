...
export function useAnimatedSeriesPath({
  ...
  const xScaleDomain = useMemo(() => {
    ...
    const scaleWithDomain = xScale as { domain?: () => [any, any] };

    const domain = scaleWithDomain.domain?.() ?? [new Date(0), new Date(0)];
  ...
  const transitionSignature = useMemo(
    ...
    () =>
      ...
      seriesPathTransitionSignature({
        ...
        xDomainMin: xScaleDomain[0]?.getTime ? xScaleDomain[0].getTime() : (xScaleDomain[0] ?? 0),

        xDomainMax: xScaleDomain[1]?.getTime ? xScaleDomain[1].getTime() : (xScaleDomain[1] ?? 0),
