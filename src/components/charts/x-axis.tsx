...
function buildDataAlignedTicks({
  ...
  xAccessor: (d: Record<string, unknown>) => Date | number;
  xScale: (date: any) => number | undefined;
}): AxisTick[] {
  const seenLabels = new Set<string>();
  ...
  for (const index of selectEvenlySpacedIndices(data.length, targetTickCount, {
    ...
    dateLabels,
    resolveXPx,
    xAccessor: xAccessor as (d: Record<string, unknown>) => Date,
  })) {
    const point = data[index];
    if (!point) {
      continue;
    }
    const val = xAccessor(point);
    const date = val instanceof Date ? val : new Date(val);
    const label = dateLabels[index] ?? (typeof val === 'number' ? String(val) : shortDateFmt.format(date));
    if (seenLabels.has(label)) {
      continue;
    }
    seenLabels.add(label);
...
function buildDomainTicks({
  ...
  for (let i = 0; i < tickCount; i++) {
    ...
    const val = startTime + t * timeRange;
    const isNumeric = typeof startDate === 'number';
    const date = isNumeric ? new Date(val) : new Date(val); // Keep AxisTick.date as Date to avoid changing the interface
    const label = isNumeric ? String(Math.round(val * 100) / 100) : shortDateFmt.format(date);
    if (seenLabels.has(label)) {
      continue;
    }
    seenLabels.add(label);
    ticks.push({
      date,
      label,
      x: (xScale(isNumeric ? val : date) ?? 0) + marginLeft,
    });
  }
...
function domainExtendsPastData(
  ...
  if (!(domainEnd !== undefined && lastPoint !== undefined)) {
    return false;
  }
  const endMs = domainEnd instanceof Date ? domainEnd.getTime() : domainEnd;
  const lastVal = xAccessor(lastPoint);
  const lastMs = lastVal instanceof Date ? lastVal.getTime() : lastVal;
  return endMs > lastMs;
}
...
function appendProjectionTailTicks(
  ...
  const lastPoint = data.at(-1);
  const domainEnd = xScale.domain()[1];
  if (!(lastPoint !== undefined && domainEnd !== undefined)) {
    return ticks;
  }

  const lastDate = xAccessor(lastPoint);
  const startTime = lastDate instanceof Date ? lastDate.getTime() : lastDate;
  const endTime = domainEnd instanceof Date ? domainEnd.getTime() : domainEnd;
  const isNumeric = typeof domainEnd === 'number';
  ...
  for (let i = 1; i <= extraCount; i++) {
    const val = startTime + (i / (extraCount + 1)) * (endTime - startTime);
    const date = isNumeric ? new Date(val) : new Date(val);
    const label = isNumeric ? String(Math.round(val * 100) / 100) : shortDateFmt.format(date);
    if (seenLabels.has(label)) {
      continue;
    }
  ...
  const endLabel = isNumeric ? String(Math.round(endTime * 100) / 100) : shortDateFmt.format(domainEnd as Date);
  if (!seenLabels.has(endLabel)) {
    extras.push({
      date: isNumeric ? new Date(endTime) : (domainEnd as Date),
      label: endLabel,
