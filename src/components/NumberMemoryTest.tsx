...
export function NumberMemoryTest({ onNavigate }: { onNavigate: (view: string) => void }) {
  ...
  const handleNumberInput = (num: string) => {
    ...
    triggerHaptic('tap');
    setPlayerInput(prev => {
      if (sequenceRef.current && prev.length >= sequenceRef.current.length) {
        return prev;
      }
      return prev + num;
    });
  ...
  let apparatusClass = "relative w-full max-w-[600px] min-h-[480px] sm:min-h-[520px] py-8 mx-auto rounded-[32px] border flex flex-col items-center justify-between cursor-pointer transition-[border-color,background-color] duration-200 ease-out z-20 backdrop-blur-[10px]";
  ...
  const renderKeypad = () => {
    ...
    return (
      <div className="grid grid-cols-3 gap-2 sm:gap-3 w-full max-w-[260px] sm:max-w-[280px] mx-auto mt-4 mb-4 z-10">
        {keys.map((key) => {
          let btnClass = "h-12 sm:h-14 rounded-xl border border-white/10 font-['Space_Grotesk'] text-xl text-white flex items-center justify-center transition-[background-color,border-color,color,transform] hover:bg-white/10 active:bg-white/20 active:scale-95 bg-white/5";
