...
let mobileWelcomeModalSeenInMemory = false;

export function isMobileWelcomeSeenInMemory(): boolean {
  return mobileWelcomeModalSeenInMemory;
}

export function setMobileWelcomeSeenInMemory(seen: boolean) {
  mobileWelcomeModalSeenInMemory = seen;
}
...
export function WelcomeModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    setDontShowAgain(mobileWelcomeModalSeenInMemory);
  }, [isOpen]);
  ...
  const handleClose = () => {
    triggerHaptic('tap');
    setMobileWelcomeSeenInMemory(dontShowAgain);
    onClose();
  };

  const handleDontShowAgain = (e: React.ChangeEvent<HTMLInputElement>) => {
    setDontShowAgain(e.target.checked);
    setMobileWelcomeSeenInMemory(e.target.checked);
  };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          ...
          <motion.div
            ...
            className="relative z-10 w-full max-w-[420px] bg-[#0b101b] border border-white/20 rounded-2xl p-5 sm:p-6 shadow-2xl flex flex-col font-sans text-[#e8ecf3] my-auto select-none"
          ...
          >
            ...
            <div className="flex items-center justify-between gap-3 mb-4">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <div className="text-[11px] text-[#8b9bb4] tracking-tight truncate">Precision User Latency &amp; Evaluator</div>
                </div>
              </div>
            ...
            <h2 className="text-lg font-bold text-white mb-1.5">Welcome to PULSE - Precision User Latency &amp; Stimulus Evaluator</h2>
            ...
            <div className="flex">
              ...
              >
                Tap Start assessments to begin
              </button>
            </div>
          </motion.div>
        </div>
